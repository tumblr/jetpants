...
* No auto_increment columns. Use an external ID generation service instead. Build your own (it's a hundred lines of C) or use Memcached or Redis or Snowflake or any other number of options.
* Primary key of all sharded tables begins with the sharding key. This takes advantage of InnoDB clustered indexes to allow efficient export/import of data.
* No schema changes in process of split. Better to avoid mixing complex operations.
...
* Redundant rows temporarily on the wrong shard don’t matter to your application. This would only matter if you're doing aggregate queries (like COUNTs) over the whole data set... but if your data set is big enough to shard, these already don't scale in InnoDB, so it shouldn't really matter.
