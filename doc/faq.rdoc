...
Absolutely! You can even use it to implement some simple map/reduce in MySQL:

   Jetpants.topology.shards.concurrent_map {|s| s.query_return_first_value 'SELECT MAX(id) from foo'}.reduce(0) {|overall_max, val| val > overall_max ? val : overall_max}
