...
We perform the copy operation using a combination of tar (for archiving), a configurable compression tool (pigz by default), and nc (for transferring the data over the network). If there are multiple destinations, we create a serial "copy chain" using tee and a fifo.
