...
* All database server hosts that are "spares" (not yet in use, but ready for use in shard splits, shard cutover, or slave cloning) need to have a STATUS of PROVISIONED. These nodes must meet the requirements of spares as defined by the REQUIREMENTS doc that comes with \Jetpants. They should NOT have a POOL set, but they SHOULD have a ROLE set to either MASTER or STANDBY_SLAVE, depending on their intended use. The role will be used to select spare nodes for shard splits, shard cutover, and slave cloning.
