...
* Jetpants::DB#before_disable_binary_logging and Jetpants::DB#before_enable_binary_logging
  * \Jetpants disables binary logging by commenting-out two lines of your my.cnf and then restarting MySQL. If your my.cnf file is controlled by a configuration management tool like Puppet or Chef, you may need to implement these callback methods to temporarily disable configuration management on the node.
