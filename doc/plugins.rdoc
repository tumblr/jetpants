...
Other recommended uses of plugins include integration with your site's monitoring system, trending system, query killer(s), and environment-specific overrides to various core methods.

== Asset tracker

=== Example

We supply a sample plugin called simple_tracker, demonstrating how to go about writing a very basic asset-tracking plugin. This plugin just stores database topology information to a JSON file, and writes a production configuration to a YAML file. This isn't actually suitable for production use, but should provide a reasonable starting point for learning the plugin system and building a real asset tracker.
...
* Jetpants::Pool#after_remove_slave!
    * If your implementation of Pool#sync_configuration works by iterating over the current slaves of the pool's master, it won't correctly handle the <tt>jetpants destroy_slave</tt> command,
      or anything else that calls DB#disable_replication! which does a RESET SLAVE. You can instead handle that special case here.
...
* If this is raise in an after_foo method, all subsequent lower-priority after_foo stacked callbacks will be skipped. The return value of foo will be unaffected.